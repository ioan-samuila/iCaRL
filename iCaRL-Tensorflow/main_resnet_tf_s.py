...
######### Paths  ########################
devkit_path = 'C:\\Users\\ioan\\Downloads\\ILSVRC\\dev_kit'
train_path  = 'C:\\Users\\ioan\\Downloads\\ILSVRC\\data'
save_path   = 'C:\\Users\\ioan\\Documents\\icarl\\backup\\'
#########################################
...
#Variate the temperature parameter
tprt = 2
print('{} - Temperature: {}'.format((datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), tprt))

def softmax(value):
  return np.exp(value/tprt) / np.sum(np.exp(value/tprt), axis=0)

### Preparing the files for the training/validation ###

# Random mixing
print((datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), " - Mixing the classes and putting them in batches of classes...")
np.random.seed(1993)
...
print((datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), " - Creating a validation set ...")
...
for itera in range(nb_groups):
  ...
  print('{} - Batch of classes number {} arrives ...'.format((datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), itera+1))
  ...
  with tf.compat.v1.Session(config=config) as sess:
    ...
    for epoch in range(epochs):
        print("{} - Batch of classes {} out of {} batches".format(
                (datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), itera + 1, nb_groups))
        ...
        for i in range(int(np.ceil(len(files_from_cl)/batch_size))):
            ...
            if len(loss_batch) == 10:
                print("{} - Training error every 10 batches: {}".format((datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), np.mean(loss_batch)))
                loss_batch = []
            ...
            if (i+1)%80 == 0:
                ...
                print("{} - Training top 1 accuracy every 80 batches {}".format((datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), stat))
  ...
  with tf.compat.v1.Session(config=config) as sess:
    ...
    print((datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), ' - Exemplars selection starting ...')
  ...
  print((datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")), ' - Computing theoretical class means for NCM and mean-of-exemplars for iCaRL with softmax ...')
  for iteration2 in range(itera+1):
      ...
      with tf.compat.v1.Session(config=config) as sess:
          ...
          for iter_dico in range(nb_cl):
              ...
              # Normal NCM mean
              class_means[:,order[iteration2*nb_cl+iter_dico],1,itera] = np.array(softmax(D[:,0])) #softmax for NCM
              ...
              # iCaRL approximated mean (mean-of-exemplars) # but we use softmax
              ...
              D_tmp       = D[:,ind_herding]
              class_means[:,order[iteration2*nb_cl+iter_dico],0,itera] = np.array(softmax(D_tmp[:,0]))
